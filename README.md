...
The example mirrors `https://book.jingjing2222.com/`.

## Releasing

Releases are intentionally manual. After merging the release-ready changes to
`main`, open the **Release** workflow in GitHub Actions, choose **Run workflow**,
and enter the SemVer version to publish. The initial stable release defaults to
`0.0.1`.

The workflow runs the full check, lets `release-it` update the package version,
creates the commit and `v<version>` tag, publishes to npm through trusted
publishing, and creates the GitHub release. It does not use an npm token.
