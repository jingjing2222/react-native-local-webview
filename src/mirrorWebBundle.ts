...
function resolveDocumentRedirectUrl(value: string, currentDocumentUrl: string): string {
  ...
  if (!value.includes('#')) {
    resolved.hash = new URL(currentDocumentUrl).hash;
  }
...
function documentUrlForRequest(
  ...
  resolved.hash = documentFragmentInherited ? new URL(requestedUrl).hash : documentFragment;
  return resolved.toString();
}
