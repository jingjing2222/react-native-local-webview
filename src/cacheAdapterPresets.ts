...
function createBlobUtilDownload(
  ...
  return async (options: LocalWebViewDownloadOptions): Promise<LocalWebViewDownloadResult> => {
    ...
    if (options.maxBytes !== undefined) {
      ...
      requestHeaders.Range = `bytes=0-${options.maxBytes}`;
    }
    const task = blobUtil
      .config({
    ...
    const inspectResponseLimit = (info: BlobUtilResponseInfo): void => {
      ...
      if (earlyResponseError || earlyResponseInfo || limitExceeded) task.cancel();
    };
    task.stateChange?.(inspectResponseLimit);
