...
export const LocalWebView = forwardRef<LocalWebViewHandle, LocalWebViewProps>(function LocalWebView(
  ...
  const streamLocalAsset = useCallback(
    async (streamKey: string, requestId: string, url: string): Promise<void> => {
      ...
      try {
        ...
        for (let position = 0; position < asset.size; position += ASSET_CHUNK_BYTES) {
          if (
            bundleEpochRef.current !== bundleEpoch ||
            cancelledAssetRequestsRef.current.has(streamKey)
          ) {
          ...
          const length = Math.min(ASSET_CHUNK_BYTES, asset.size - position);
  ...
  const handleMessage = (event: WebViewMessageEvent) => {
    ...
    if (asset) {
      ...
      if (asset.kind === 'request' && asset.url) {
        void streamLocalAsset(streamKey, asset.requestId, asset.url);
