...
(() => {
  ...
  const byteRange = (value, size) => {
    if (value === null || value === undefined || String(value).trim() === '') {
      return { end: size, start: 0 };
    }
    ...
    if (!match[1]) {
      ...
      const inclusiveEnd = match[2] ? Number(match[2]) : size - 1;
      if (!Number.isSafeInteger(inclusiveEnd)) return null;
      end = Math.min(size, inclusiveEnd + 1);
    }
  ...
  const receive = (event) => {
    ...
    if (message.kind === 'chunk') {
      try {
        ...
        if (request.receivedBytes > request.expectedBytes) {
    ...
    finishRequest(message.requestId, request);
    if (message.kind === 'end' && request.receivedBytes === request.expectedBytes) {
  ...
  const resolveAsset = (input, integrityMetadata = '', signal = null, rangeHeader = null) => {
    let url;
    try {
      ...
      return exposeResponseMetadata(
        new Response(new Uint8Array(0), {
          headers: {
            'Accept-Ranges': 'bytes',
            'Content-Length': '0',
            'Content-Range': 'bytes */' + String(asset.size),
    ...
    const request = {
      ...
      expectedBytes: range.end - range.start,
      nativeStarted: false,
      receivedBytes: 0,
      start: range.start,
      url,
    ...
    return exposeResponseMetadata(
      new Response(stream, {
        headers: {
          ...
          'Content-Type': asset.mediaType,
          ...(range.start !== 0 || range.end !== asset.size
            ? { 'Content-Range': 'bytes ' + String(range.start) + '-' + String(range.end - 1) + '/' + String(asset.size) }
            : {})
        },
        status: range.start !== 0 || range.end !== asset.size ? 206 : 200,
        statusText: range.start !== 0 || range.end !== asset.size ? 'Partial Content' : 'OK'
      }),
      asset.responseUrl || url,
      Boolean(asset.redirected)
    );
  ...
  const installWorkerRelay = (endpoint, closeMethodName) => {
    ...
    const receiveWorkerRequest = (event) => {
      ...
      if (
        ...
        || (request.range !== undefined && typeof request.range !== 'string')
      ) {
        try {
          postToRelay(relay, {
            kind: 'error',
  ...
  window.fetch = function localAssetFetch(resource, init) {
    ...
    try {
      response =
        method.toUpperCase() === 'GET' ? resolveAsset(input, integrity, signal, range) : null;
    } catch (error) {
  ...
  if (typeof NativeXMLHttpRequest === 'function' && typeof EventTarget === 'function') {
    ...
    class LocalAssetXMLHttpRequest extends EventTarget {
      ...
      send(body = null) {
        ...
        const response = resolveAsset(this.local, '', null, this.requestHeaders.range || null);
        if (!response) {
          this.resetLocalResponse();
          this.transition(4);
          this.emit('error');
        ...
        void (async () => {
          try {
            ...
            if (!Number.isSafeInteger(responseSize) || responseSize < 0) {
              throw new Error('Local asset response has an invalid content length');
            }
            const merged = new Uint8Array(responseSize);
            let received = 0;
            while (true) {
              const result = await reader.read();
              if (this.requestEpoch !== requestEpoch) return;
    ...
    // Web IDL exposes XMLHttpRequest's public prototype surface as enumerable.
    // Legacy Unity loaders depend on property enumeration to proxy this surface.
    for (const name of Object.getOwnPropertyNames(LocalAssetXMLHttpRequest.prototype)) {
      if (name === 'constructor') continue;
      const descriptor = Object.getOwnPropertyDescriptor(
        LocalAssetXMLHttpRequest.prototype,
        name
      );
      if (descriptor) {
        Object.defineProperty(LocalAssetXMLHttpRequest.prototype, name, {
          ...descriptor,
          enumerable: true,
        });
      }
    ...
    for (const [name, value] of [
      ...
      Object.defineProperty(LocalAssetXMLHttpRequest, name, { enumerable: true, value });
      Object.defineProperty(LocalAssetXMLHttpRequest.prototype, name, {
        enumerable: true,
        value,
      });
