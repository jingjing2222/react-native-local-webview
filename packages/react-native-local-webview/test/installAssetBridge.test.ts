...
describe('local asset runtime bridge', () => {
  ...
  it('serves an empty verified asset without opening a native stream', async () => {
    const emptyUrl = 'https://game.example/play/empty.data';
    const emptyAssets: Record<string, AssetBridgeDescriptor> = {
      [emptyUrl]: {
        ...assets[assetUrl]!,
    ...
    const { source } = injectedScript(
      installAssetBridge('<!doctype html><html><head></head><body></body></html>', emptyAssets)
    );
    const runtime = runtimeFor(source);
    const localFetch = runtime.context.fetch as typeof fetch;

    const response = await localFetch(emptyUrl);
  ...
  it('serves a single byte range from the verified local asset', async () => {
    const { source } = injectedScript(
      installAssetBridge('<!doctype html><html><head></head><body></body></html>', assets)
    );
    const runtime = runtimeFor(source);
    ...
    expect(request).toMatchObject({ end: 3, start: 1, url: assetUrl });
    ...
    const body = response.arrayBuffer();
    runtime.dispatchNative(nativeMessage(requestId, { data: 'YXM=', kind: 'chunk' }));
    runtime.dispatchNative(nativeMessage(requestId, { kind: 'end' }));

    expect(new Uint8Array(await body)).toEqual(new Uint8Array([97, 115]));
  });

  it('returns 416 without opening a native stream for an invalid byte range', async () => {
    const { source } = injectedScript(
      installAssetBridge('<!doctype html><html><head></head><body></body></html>', assets)
    );
    const runtime = runtimeFor(source);
  ...
  it('preserves a bodyless 416 response through XMLHttpRequest and Worker relays', async () => {
    const { source } = injectedScript(
      installAssetBridge('<!doctype html><html><head></head><body></body></html>', assets)
    );
    const runtime = runtimeFor(source);
    ...
    xhr.setRequestHeader('Range', 'bytes=9-10');
    xhr.responseType = 'arraybuffer';
    const loaded = new Promise<void>((resolve, reject) => {
      xhr.onload = () => resolve();
      xhr.onerror = () => reject(new Error('416 XHR bridge failed'));
    });
    xhr.send();
    await loaded;

    expect(xhr.status).toBe(416);
    expect((xhr.response as ArrayBuffer).byteLength).toBe(0);
    ...
    worker.dispatchFromWorker(
      {
        ...
        kind: 'request',
        range: 'bytes=9-10',
        requestId: 'worker:invalid-range',
        url: assetUrl,
      },
      [relayPort]
    );
    ...
    expect(relayPort.posted.some(({ message }) => message.kind === 'error')).toBe(false);
    expect(runtime.outbound).toEqual([]);
  });
  ...
  it('keeps the XMLHttpRequest surface enumerable for Unity cache wrappers', () => {
    const { source } = injectedScript(
      installAssetBridge('<!doctype html><html><head></head><body></body></html>', assets)
    );
    const runtime = runtimeFor(source);
    ...
    for (const property in xhr) properties.push(property);
