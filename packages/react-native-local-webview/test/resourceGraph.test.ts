...
describe('complete CSR asset graph', () => {
  it('does not inherit React Native URL base-resolution semantics', async () => {
    const NativeUrl = globalThis.URL;
    class ReactNativeUrl {
      readonly href: string;

      constructor(value: string, base?: string) {
        this.href = base ? `${base.replace(/\/$/, '')}/${value.replace(/^\//, '')}` : value;
      }
    ...
    Reflect.set(globalThis, 'URL', ReactNativeUrl);
    try {
      const seen = new Set<string>();
      await localizeWebDocument({
        entryUrl: 'https://app.example/releases/current/index.html',
        html: '<!doctype html><html><head><script src="/assets/app.js"></script></head></html>',
        load: loader(
          [
            resource(
      ...
      expect(seen).toEqual(new Set(['https://app.example/assets/app.js']));
    } finally {
  ...
  it('localizes a generic Unity WebGL loader and streams its large runtime files', async () => {
    ...
    const frameworkSource = dynamicScripts.sources.get(dynamicScripts.materialize(frameworkUrl));
    expect(frameworkSource).toBe('globalThis.unityFramework = true;');
  ...
  it('builds executable Worker catalogs when the host runtime hides function source', async () => {
    const originalToString = Object.getOwnPropertyDescriptor(Function.prototype, 'toString')!;
    let html = '';
    Object.defineProperty(Function.prototype, 'toString', {
      ...originalToString,
      value: () => 'function hostBytecode() { [bytecode] }',
    });
    try {
      const origin = 'https://hermes-worker.example';
      const workerUrl = `${origin}/probe.worker.js`;
      const result = await localizeWebDocument({
        entryUrl: `${origin}/index.html`,
        html: '<!doctype html><script>new Worker("/probe.worker.js");</script>',
        load: loader([resource(workerUrl, 'postMessage("ready");', 'text/javascript')]),
      });
      html = result.html;
    } finally {
      Object.defineProperty(Function.prototype, 'toString', originalToString);
    }
    expect(html).not.toContain('[bytecode]');
    const graph = materializedWorkerGraph(html);
    const workerCode = graph.sources.get(graph.materialize(graph.rootIds[0]!));
