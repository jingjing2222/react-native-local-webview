...
describe('LocalWebView loading lifecycle', () => {
  ...
  it('reads only the byte range requested by the local asset bridge', async () => {
    const mirrored = streamBundle('range', 1);
    const localRead = deferred<string>();
    runtime.resolveWebBundle.mockResolvedValue(mirrored);
    runtime.readMirroredWebBundle.mockResolvedValue(
      '<!doctype html><html><head></head><body>range</body></html>'
    );
    adapterReadFileRange.mockReturnValue(localRead.promise);

    let renderer!: TestRenderer.ReactTestRenderer;
    await act(async () => {
      renderer = TestRenderer.create(<LocalWebView virtualUrl="https://range.example/" />);
    });
    await flush();

    await act(async () => {
      sendAssetMessage(webView(renderer), {
        end: 3,
        kind: 'request',
        requestId: 'range-request',
        start: 1,
        url: 'https://range.example/asset-0.data',
      });
      await Promise.resolve();
    });

    expect(adapterReadFileRange).toHaveBeenCalledWith('/documents/asset-0.data', 1, 3, 'base64');

    await act(async () => {
      renderer.unmount();
      localRead.resolve('YXM=');
      await Promise.resolve();
    });
  });
