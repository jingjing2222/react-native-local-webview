...
describe('resolveWebBundle', () => {
  ...
  it('refreshes after a proven change even when a cancelled sibling rejects generically', async () => {
    const first = await resolveWebBundle({ virtualUrl: ENTRY });
          ...
          return await new Promise((_resolve, reject) => {
            options.signal?.addEventListener(
              ...
              () => {
                ...
                reject(new Error('provider cancelled without an AbortError name'));
              },
              { once: true }
            );
          });
  ...
  it('waits for every revalidation sibling before returning an offline fallback', async () => {
    const first = await resolveWebBundle({ virtualUrl: ENTRY });
    let releaseScript!: () => void;
    let markScriptStarted!: () => void;
    ...
    let settled = false;
    const resolving = resolveWebBundleWithAdapter({
  ...
  it('persists statically discoverable Unity WebGL runtime files', async () => {
    ...
    expect(localized).not.toContain(`${entryUrl}Build/game.framework.js`);
    expect(localized).toContain('data:text/javascript;base64,');
